...
import signal
import sys
from shadowservice import SentryShadow
...
def sigterm_handler(signum, frame):
    logging.info("Signal to terminate sentry shadow service received")
    # shutdown_sentry_service()

signal.signal(signal.SIGTERM, sigterm_handler)

if os.path.exists("/tmp/sentry_shadow_service_unix_socket"):
    os.remove("/tmp/sentry_shadow_service_unix_socket")

logging.info("Opening socket...")

server = socket.socket(socket.AF_UNIX, socket.SOCK_DGRAM)
server.bind("/tmp/sentry_shadow_service_unix_socket")

last_fire_time = 0

def shutdown_sentry_shadow_service():
    # sentry_controller.shutdown()
    server.close()
    os.remove("/tmp/sentry_shadow_service_unix_socket")
    logging.info("Shutdown sentry shadow service, cleaned up, Shutting down...")
    sys.exit()
